...
class ChromeDriver(object):
    ...
    def __init__(self, privacy_proxy=None, proxy=None,
                 headless=False, driver_name='mac_chromedriver_96.0.4664.45'):
        ...
        # 私密代理
        if privacy_proxy:
            self.options.add_extension(self.get_chrome_proxy_extension(proxy=privacy_proxy))
        # 普通代理
        if proxy:
    ...
    def get_chrome_proxy_extension(self, proxy):
        ...
        custom_chrome_proxy_extensions_dir = os.path.join(self.up_path, 'chrome-proxy-extensions', '')
    ...
    def trace_and_move(self, slider, x, y, ):
        """
            trace_and_move
        ...
        ActionChains(self.driver).drag_and_drop_by_offset(slider, xoffset=(y - 56) / 2, yoffset=0).perform()
